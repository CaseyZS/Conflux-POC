...
- **Project** — a body of work for one client. Fields: name, `billing type`, `billing method` (only when hourly), `hourly rate` (used when hourly + per-project), `fixed fee` amount + nullable `fixed_fee_invoice_id` (the fee's anti-double-bill link, set at finalize), status (active/archived). Currency is inherited from the client, not stored again. Belongs to a client; owns its task assignments **and its purchase orders** (see next).
- **Purchase Order (PO)** — a client-authorized spending limit against a project (requirement captured 2026-07-03). A project can have **several** POs; each is managed from the project-editing screen. Fields: a `reference` (the client's PO number/label), an authorized `amount` (integer minor units, G4), and a `status` (active/archived). Currency is **inherited from the project's client**, never stored per PO (same rule as everywhere else — [derive, don't duplicate](#rate-storage-derive-dont-duplicate-until-finalized)). Its **remaining funds are derived, not stored** — the authorized amount minus what finalized (non-void) invoices have billed against it, never a stored running balance (guardrail G5 / the duplication trigger A). Org-scoped child of the Project, archived with it (G12). Invoices bill against a PO — the selection lives on the Invoice↔Project selection (below), and per-PO consumption / overdraw handling is defined in [invoicing](./invoicing.md).
...
- **Invoice** — a bill to one **Client** (org-scoped). Fields: status (`draft` → `sent` → `paid`), invoice number (assigned at finalize), issue date, due date (defaults to issue + terms, **user-overridable**), payment terms, discount, tax, footer, and — snapshotted at finalize — the **currency**, money totals, and frozen **bill-to** (client name/contact/address) and **from/branding** (org name/address/contact; logo by reference) blocks. Has many `InvoiceLine`s. The **PO** the invoice bills against is chosen **per included project** on the Invoice↔Project selection (below), superseding the former single free-text "PO number" field; the selected PO reference(s) are frozen into the snapshot at finalize. Full lifecycle in [invoicing](./invoicing.md).
...
- **Invoice ↔ Project selection** — the draft's stored choice of which of the client's projects feed the invoice ([invoicing](./invoicing.md): "a draft stores its choices"), with two flags per selected project: pull its unbilled billable time, and/or include its fixed fee, plus a **nullable chosen PO** — which of that project's POs this invoice bills against (null = no PO). This is the seam that makes per-PO tracking coherent when one invoice spans several projects (each project's portion draws on its own PO). Deleted with its draft (cascade — it's a choice, not financial history); the anti-double-bill links on time entries and the fixed fee — and the PO's consumption, derived from finalized invoices — are separate and settle only at finalize.
...
erDiagram
    ...
    PROJECT ||--o{ PURCHASE_ORDER : authorizes
    ...
    PURCHASE_ORDER ||--o{ INVOICE_PROJECT : "billed against (nullable)"
```
