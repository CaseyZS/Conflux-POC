...
| D18 | Hosting: **Railway** — an always-on service + managed Postgres in one project, Conflux as tenant #1 of a shared personal-project platform | Two-way | The decision driver is a flexible home for several personal projects, not this one app; an always-on flat-fee box amortizes across them and runs headless Chromium (the PDF path, G9) without serverless size/cold-start workarounds. The ORM boundary (G8) keeps the SQLite→Postgres switch a config change, so the host stays swappable ([deployment](./deployment.md)). | 2026-07-03 |
| D19 | **Purchase Orders are first-class, project-owned** (a project has many POs, each with an authorized amount); an invoice picks a PO **per included project** (the ref rides on the Invoice↔Project selection), each PO's **remaining funds are derived** (amount − finalized non-void billings), and **overdraw is warn-but-allow** policy | Two-way | Promotes the old free-text invoice "PO number" to an entity so funds track per PO, not just per project. Deriving remaining honors G5/trigger A (void releases consumption for free, like the anti-double-bill links); the per-project selection (the deliberate fork over per-invoice / per-line) fits multi-project invoices and keeps attribution coherent; overdraw-as-policy honors G6. Additive — the PO ref rides the existing selection and finalize snapshot; a post-POC feature, not in M0–M5 ([data model](./requirements/data-model.md), [invoicing](./requirements/invoicing.md)). | 2026-07-03 |
