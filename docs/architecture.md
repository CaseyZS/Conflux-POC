...
| G4  | **Money is stored as integer minor units** (cents); formatting happens only at display.            | Avoids floating-point rounding on currency — non-negotiable for anything that bills.                       |
