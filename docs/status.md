...
- [ ] **Seg 4** — weekly grid: projects×days cells for review and bulk entry. Rolls in the 2026-07-02 display feedback first (the grid renders durations in every cell, so format lands before the grid): **H:MM replaces decimal hours** as the default duration display, duration input accepts both `1:30` and `1.5`, the live clock drops seconds, and a same-day resume skips the fresh/continue chooser (both paths land on the same day — it just continues). Decimal hours return as a choice when the M5 settings page adds the time-format preference.
